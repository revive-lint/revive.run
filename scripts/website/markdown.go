...
// splitSections drops everything before the first "## " heading (title, intro, table of contents)
// and splits the rest of the document into sections, one per "## " heading. Headings inside code fences are ignored.
...
var (
	...
	// rulesFilePlainLinkRE matches "./RULES_DESCRIPTIONS.md" links without a fragment.
	rulesFilePlainLinkRE = regexp.MustCompile(`\]\(\./RULES_DESCRIPTIONS\.md\)`)
	// readmeLinkRE matches "./README.md" and "./README.md#configuration" links,
	// capturing the optional fragment.
...
// rewriteRuleLinks rewrites the cross-references of a RULES_DESCRIPTIONS.md section to the URL scheme of the website.
// Absolute URLs and same-page fragment links are left untouched.
func rewriteRuleLinks(body string) string {
	body = rulesFileLinkRE.ReplaceAllString(body, "](#${1})")
	body = readmeLinkRE.ReplaceAllString(body, "](/docs/${1})")
	return body
}

// transformReadme turns the README.md content into the body of the /docs/ page: it drops the leading H1
// and the table-of-contents block, and rewrites repository-relative references. Absolute URLs are left untouched.
...
// stripTocBlock removes the markdown-toc block, "<!-- toc -->" through "<!-- tocstop -->" inclusive.
// The document is returned unchanged if the markers are absent.
...
// stripLeadingH1 removes the document's H1 heading if it is the first non-blank line.
...
type frontMatter struct {
	title       string
	description string
	url         string // page URL, when different from the content path
	weight      int    // position in the sidebar
	icon        string // Material Symbols icon name shown in the sidebar
}
...
func pageContent(fm frontMatter, body string) string {
	...
	if fm.url != "" {
		fmt.Fprintf(&b, "url: %s\n", strconv.Quote(fm.url))
	}
	if fm.weight != 0 {
		fmt.Fprintf(&b, "weight: %d\n", fm.weight)
	}
	if fm.icon != "" {
		fmt.Fprintf(&b, "icon: %s\n", strconv.Quote(fm.icon))
	}
	b.WriteString("---\n\n")
	b.WriteString(body)
...
// rulesPageContent renders the body of the single /r/ page: a short intro followed by every
// RULES_DESCRIPTIONS.md section as an "## " heading in documentation order.
// Heading anchors are GitHub-style slugs, so legacy /r#<rule-name> and /r/#<rule-name> links keep working with no redirects.
